...
inline int core_calc(int st,int ed)
{
	...
	for(int i=st,xx,yy;i<=ed;i++)
	{
		xx=lower_bound(arr+st,arr+ed+1,tar-arr[i])-arr;
		//yy=lower_bound(arr+st,arr+ed+1,tar-arr[i])-arr;
		if(arr[xx]+arr[i]==tar)
		{
			while(arr[xx]+arr[i]==tar)
			{
				ret++;
				xx++;
			}
			//if(xx!=yy) cerr<<"Fuck you!"<<endl;
			//ret+=yy-xx;
		}
	}
