...
inline int fastpow(int x,int tme,int mod,bool &abo)
{
    ...
    if(ret>=mod) abo=1; // you will wa without this!
    return int(ret%mod);
}
...
{
    if(!dpt) return fastpow(in[pos],1,phi[modd],labo);
    bool myabo=0;
    lli tmp=solve(pos,dpt-1,modd+1,myabo);
    if(myabo) return fastpow(c,tmp+phi[modd+1],phi[modd],labo);
...
void update(int l,int r,int pos)
{
    ...
    if(!lazy[lson[pos]]) update(l,mid,lson[pos]);
    if(!lazy[rson[pos]]) update(mid+1,r,rson[pos]);
    dat[pos]=dat[lson[pos]]+dat[rson[pos]];
