...
{
    debug<<"x="<<x<<"ope="<<ope<<endl;
    if(!x) puts("Fuck you"),exit(0);
    for(int i=1;i<=pl;i++)
    {
        while(!(x%prime[i]))
        {
...
int main()
{
    ...
    debug<<"updated finished with argument (n-2,1)"<<endl;

    for(int i=1;i<=n-2-tot;i++) mul(m);

    debug<<"divide with 1 finished"<<endl;


    debug<<"n-2-tot="<<n-2-tot<<endl;
    ...
    for(int i=1;i<=n;i++) if(in[i]) divide(in[i],-1);

    debug<<"all update finished"<<endl;
