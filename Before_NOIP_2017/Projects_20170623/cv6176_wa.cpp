#include<iostream>
#include<cstdio>
#include<cmath>
#include<cstring>
#include<algorithm>
#define debug cout
using namespace std;
const int maxn=3197152;
struct Comp
{
	double r,i;
	Comp(double rr=0,double ii=0) { r=rr,i=ii;}
	Comp conj() { return Comp(r,-i);}
	double real() { return r;}
	friend Comp operator * (const Comp &a,const Comp &b) { return Comp(a.r*b.r-a.i*b.i,a.r*b.i+a.i*b.r);}
	friend Comp operator + (const Comp &a,const Comp &b) { return Comp(a.r+b.r,a.i+b.i);}
	friend Comp operator - (const Comp &a,const Comp &b) { return Comp(a.r-b.r,a.i-b.i);}
};

Comp a[maxn],b[maxn],eps[maxn],anti_eps[maxn];
char in[maxn];
int out[maxn],len,k;

inline void geteps(int len)
{
	const double pi=acos(-1.0);
	for(int k=0;k<len;k++) eps[k]=Comp(cos(pi*2*k/len),sin(pi*2*k/len)),anti_eps[k]=eps[k].conj();
}

void bitrev(int n,Comp* x)
{
	for(int i=0,j=0;i<n;i++)
	{
		if(i>j) swap(x[i],x[j]);
		for(int l=n>>1;(j^=l)<l;l>>=1);
	}
}

inline void iter_fft(int len,Comp* x,Comp* eps)
{
	bitrev(len,x);
	for(int i=2;i<=len;i<<=1) // i is the step
	{
		int m=i>>1;
		for(int j=0;j<len;j+=i) // j is the calcing pos?
			for(int k=0;k<m;k++) // k is calcing length
			{
				Comp z=x[j+m+k]*eps[len/i*k];
				x[j+m+k]=x[j+k]-z;
				x[j+k]=x[j+k]+z;
			}
	}
}

inline int convert(char x)
{
    if('0'<=x&&x<='9') return x-'0';
    return x-'A'+10;
}
inline char reconvert(int x)
{
    if(x<=9) return x+'0';
    return x-10+'A';
}


inline void getput()
{
	int t,maxt;
	scanf("%s",in);
	maxt=t=strlen(in);
	for(int i=0,j=t-1;i<t;i++,j--) a[j]=Comp(convert(in[i]),0);
	scanf("%s",in);
	maxt=max(maxt,t=strlen(in));
	for(int i=0,j=t-1;i<t;i++,j--) b[j]=Comp(convert(in[i]),0);
	t=0;
	while((1<<t)<maxt) t++;
	len=1<<(t+1);
}

int main()
{
    scanf("%d",&k);
	getput();
	geteps(len);
	iter_fft(len,a,eps);
	iter_fft(len,b,eps);
	for(int i=0;i<len;i++) a[i]=a[i]*b[i];
	iter_fft(len,a,anti_eps);
	for(int i=0;i<len;i++) out[i]=(a[i].real()+0.1)/len;
	for(int i=0;i<len;i++) out[i+1]+=out[i]/k,out[i]%=k;
	while((!out[len])&&len>=0) len--;
	if(len<0) putchar('0');
	//debug<<"to output"<<endl;
	while(len>=0) putchar(reconvert(out[len])),len--;
	putchar('\n');
	return 0;
}
