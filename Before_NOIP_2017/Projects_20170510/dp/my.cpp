...
lli in[maxn],phi[100];
int n,m,p,c;
int l[8*maxn],r[8*maxn],lson[8*maxn],rson[8*maxn],lazy[8*maxn],tme[8*maxn],cnt;
lli dat[8*maxn];
...
void update(int ll,int rr,int pos)
{
    ...
    if(!lazy[lson[pos]]) update(ll,rr,lson[pos]);
    if(!lazy[rson[pos]]) update(ll,rr,rson[pos]);
    dat[pos]=dat[lson[pos]]+dat[rson[pos]];
    lazy[pos]=lazy[lson[pos]]&&lazy[rson[pos]];
}
lli query(int ll,int rr,int pos)
{
    if(rr<l[pos]||r[pos]<ll) return 0;
    if(ll<=l[pos]&&r[pos]<=rr) return dat[pos];
    return query(ll,rr,lson[pos])+query(ll,rr,rson[pos]);
}
