...
struct SegmentTree
{
	...
	inline void push(int pos)
	{
		if(lazy[pos])
		{
			if(lson[pos]) lazy[lson[pos]]+=lazy[pos],dat[lson[pos]]+=lazy[pos];
			if(rson[pos]) lazy[rson[pos]]+=lazy[pos],dat[rson[pos]]+=lazy[pos];
			lazy[pos]=0;
		}
	}
	...
	inline int query(int pos,int ll,int rr)
	{
		...
		if(ll<=l[pos]&&r[pos]<=rr)
		{
			...
			int ret=upper_bound(nums+st[pos],nums+ed[pos]+1,dat[pos])-(nums+st[pos]);
			//ret--;
