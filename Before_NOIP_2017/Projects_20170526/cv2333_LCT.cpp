...
{
	node *fa,*ffa;
	//if(!x->fa) debug<<"(in splay)returning"<<endl;
	push_up(x);
	while(x->fa)
	{
		fa=x->fa;
		if(x!=fa->lson&&x!=fa->rson) break;
		debug<<"calcing ffa"<<endl;
		ffa=fa->fa;
		debug<<"x="<<x-splay<<"fa="<<fa-splay<<"ffa="<<ffa-splay<<endl;
		debug_node(x);
		debug_node(fa);
		debug_node(ffa);
		if(fa!=ffa->lson&&fa!=ffa->rson) ffa=NULL;
		if(!ffa)
		{
			debug<<"!ffa"<<endl;
			if(x==fa->lson) zig(x);
...
inline void access(node *x)
{
    ...
    while(x)
    {
    	//debug<<"(in access)x="<<x-splay<<endl;
    	//debug<<"t="<<t-splay<<endl;
        splay_to_root(x);
        //debug<<"splayed"<<endl;
        x->rson=t;
        t=x;
...
inline void link(node *x,node *y)
{
    ...
    x->fa=y;
    splay_to_root(x);
    debug<<"x="<<x-splay<<"fa="<<x->fa-splay<<"ffa="<<x->fa->fa-splay<<endl;
    //debug<<"fal="<<x->fa->fa->lson-splay<<"far="<<x->fa->fa->rson-splay<<endl;
}
inline void cut(node *x,node *y)
{
    ...
    access(y);
    splay_to_root(y);
    debug<<"(in cut)x->fa="<<x->fa-splay<<endl;
    x->fa=NULL;
    //debug<<"y="<<y-splay<<"lson="<<y->lson-splay<<"rson="<<y->rson-splay<<endl;
    y->lson=NULL;
}
int main()
{
