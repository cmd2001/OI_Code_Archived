...
#include<cstring>
#include<algorithm>
const int maxn=5e2+1e1,maxe=maxn*maxn;
...
int main() {
    while( scanf("%d",&n) == 1 ) {
        ufs.init() , m = gcnt = 0 , memset(group,0,sizeof(group)) , ans = inf;
        ...
        for(int i=1;i<=gcnt;i++) if( group[i] ) ans = std::min( ans , ns[group[i]].val + ns[bin(group[i])].val );
        printf("%d\n",ans);
    }
    return 0;
}
