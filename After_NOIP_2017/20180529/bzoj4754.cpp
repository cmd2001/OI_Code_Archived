...
struct Tree {
    int s[maxn],t[maxn<<1],nxt[maxn<<1],deg[maxn],siz[maxn],cnt,root,fs;
    vector<int> sons[maxn];
    vector<ulli> prf[maxn];
    ulli h[maxn];

    inline void copystp(const vector<int> &v,const int &pos) {
        for(unsigned i=0;i<v.size();i++) {
            stp[v[i]] = h[v[i]];
            if( v[i] != pos ) stp[v[i]] += siz[v[i]] < siz[pos] ? siz[v[i]] : fs - siz[pos];
        }
    }
    inline ulli gethsh(const vector<int> &sons,vector<ulli> &prf) {
        prf.resize(sons.size());
        for(unsigned i=0;i<sons.size();i++) prf[i] = ( i ? prf[i-1] : 0 ) * base + stp[sons[i]];
        return *prf.rbegin();
    }
    ...
    inline ulli getspi(const vector<ulli> &hfa,const int &id) { // fa can't be a leaf node .
        if( id == 0 ) return getseg(hfa,1,hfa.size()-1);
            ...
            return prv * pows[hfa.size()-id-1] + nxt;
        }
    }
    ...
    inline void getsiz(int pos,int fa) {
        siz[pos] = 1;
        for(int at=s[pos];at;at=nxt[at]) if( t[at] != fa ) getsiz(t[at],pos) , siz[pos] += siz[t[at]];
    }
    inline void dfs1(int pos,int fa) {
        for(int at=s[pos];at;at=nxt[at]) if( t[at] != fa ) sons[pos].push_back(t[at]) , dfs1(t[at],pos);
        sons[pos].push_back(pos) , h[pos] = ini  , copystp(sons[pos],pos);
        std::sort(sons[pos].begin(),sons[pos].end(),cmp) , h[pos] = gethsh(sons[pos],prf[pos]);
    }
    inline void dfs2(int pos,int fa,int fid) {
        if( ~fa ) {
            sons[pos].push_back(fa);
            const ulli bk = h[fa] , oth = getspi(prf[fa],fid);
            h[fa] = oth , h[pos] = ini , copystp(sons[pos],pos);
            std::sort(sons[pos].begin(),sons[pos].end(),cmp);
            h[pos] = gethsh(sons[pos],prf[pos]) , h[fa] = bk;
        } // now h is the hash of making pos root .
        for(unsigned i=0;i<sons[pos].size();i++) if( sons[pos][i] != pos && sons[pos][i] != fa ) dfs2(sons[pos][i],pos,i);
    }
    inline void dfs3(int pos,int fa,int fid) {
        ...
        for(unsigned i=0;i<sons[pos].size();i++) if( sons[pos][i] != pos && sons[pos][i] != fa ) dfs3(sons[pos][i],pos,i);
    }

    inline void workA() {
        for(int i=1;i<n;i++) if( deg[i] > 1 ) { root = i; break; }
        fs = n - 1 , getsiz(root,-1) , dfs1(root,-1) , dfs2(root,-1,-1);
        for(int i=1;i<n;i++) app.insert(h[i]);
    }
    inline void workB() {
        for(int i=1;i<=n;i++) if( deg[i] > 1 ) { root = i; break; }
        fs = n , getsiz(root,-1) , dfs1(root,-1);
        dfs2(root,-1,-1) , dfs3(root,-1,-1);
    }
    inline void solve2() {
