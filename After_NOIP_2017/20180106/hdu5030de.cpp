...
namespace SAM {
	...
	inline void reset() {
		...
		memset(fa,0,sizeof(fa)) , memset(id,0,sizeof(id));
		memset(sa,0,sizeof(sa)) , memset(rnk,0,sizeof(rnk));
		memset(len,0,sizeof(len)) , memset(height,0,sizeof(height));
		memset(sum,0,sizeof(sum)) , memset(in,0,sizeof(in));
		memset(stk,0,sizeof(stk)) , memset(cc,0,sizeof(cc));
		li = top = root = last = cnt = sal = 0;
	}
}
