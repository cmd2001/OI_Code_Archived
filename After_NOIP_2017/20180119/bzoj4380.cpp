...
int lp[maxm],rp[maxm],vp[maxm],srt[maxm],llen;
...
int main() {
    ...
    sort(srt+1,srt+1+m);
    llen = unique(srt+1,srt+1+m) - srt - 1;
    for(int i=1;i<=m;i++)
        vp[i] = lower_bound(srt+1,srt+1+llen,vp[i]) - srt;
    
    for(int len=1;len<=n;len++)
        for(int st=1;st+len-1<=n;st++) {
            const int ed = st + len - 1;
            for(int i=st;i<=ed;i++) memset(h[i]+1,0,sizeof(int)*llen);
            for(int i=1;i<=m;i++)
                if( st <= lp[i] && rp[i] <= ed )
