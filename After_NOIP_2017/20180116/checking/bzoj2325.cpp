...
struct Node {
    ...
    inline void print() {
        debug<<"printing crs"<<endl;
        for(int i=0;i<2;i++) {
            for(int j=0;j<2;j++) debug<<crs[i][j]<<" ";
            debug<<endl;
        }
        debug<<"printing fm"<<endl;
        for(int i=0;i<2;i++) {
            for(int j=0;j<2;j++) debug<<fm[i][j]<<" ";
            debug<<endl;
        }
    }
}ns[maxn<<3];
...
inline Node query(int pos,const int &ll,const int &rr) {
    //debug<<"in query pos = "<<pos<<endl;
    debug<<"ll = "<<ll<<"rr = "<<rr<<"lp = "<<l[pos]<<"rp = "<<r[pos]<<endl;
    //if( !pos ) exit(0);
    if( ll <= l[pos] && r[pos] <= rr ) {
        debug<<"pos = "<<pos<<"l = "<<l[pos]<<"r = "<<r[pos]<<endl;
        ns[pos].print();
        return ns[pos];
    }
...
inline Node querychain(int a,int l,int flag) {// flag = 1 means include b . 
    ...
    //debug<<"top = top"<<endl;
    //debug<<"a = "<<a<<endl;
    //debug<<"flag = "<<flag<<" l = "<<num[l]+1-flag<<"r = "<<num[a]<<"cov = "<<cov[top[a]]<<endl;
...
inline Node getans(int a,int b) {
    ...
    if( a == l ) {
        ...
        //debug<<"case 3"<<endl;
        Node A = querychain(a,l,0).reverse();
        //A.print();
        Node B = querychain(b,l,1);
        //debug<<"This is B"<<endl;
        //B.print();
        //exit(0);
        return A + B;
    }
