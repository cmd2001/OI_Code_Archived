...
inline void chain(int x,int lc,const Equation &e) {
    while( top[x] != top[lc] ) {
        //debug<<"in chain l = "<<id[top[x]]<<"r = "<<id[x]<<"e = "<<e.k<<" "<<e.b<<endl;
        sgt.update(1,id[top[x]],id[x],e) ,
        usl[top[x]] = id[top[x]] , usr[top[x]] = max(usr[top[x]],id[x]) , x = fa[top[x]];
    }
    //debug<<"in chain l = "<<id[lc]<<"r = "<<id[x]<<"e = "<<e.k<<" "<<e.b<<endl;
...
inline int coresme(int t,int l,int r,const Equation &e) {
    debug<<"in coresame l = "<<l<<"r = "<<r<<endl;
...
inline int getsme(int x,int lc,const Equation &e,bool qlca) {
    ...
    while( top[x] != top[lc] ) {
        ...
        if( usl[t] && usr[t] ) { // same chain visited .
            debug<<"x = "<<x<<"top = "<<t<<endl;
            debug<<"first chain l = "<<usl[t]<<" "<<usr[t]<<endl;
...
inline bool queryb(int u,int v,lli t) {
    ...
    if( l == u ) { // fully down .
        ...
        debug<<"in queryb in else"<<endl;
        const lli tlca = t + dis[u] - dis[l];
        debug<<"lca = "<<l<<"tlca = "<<tlca<<endl;
        Equation eu = (Equation){-1,t+dis[u]} , ev = (Equation){1,tlca-dis[l]};
        sme = getsme(u,l,eu,0) + getsme(v,l,ev,1);
        debug<<"sme = "<<sme<<endl;
        //exit(0);
        if( sme ) return sme > 1;
        debug<<"we got l = "<<usl[1]<<" r = "<<usr[1]<<endl;
...
int main() {
    ...
    /*for(int i=1;i<=n;i++) debug<<dep[i]<<" "; debug<<endl;
    for(int i=1;i<=n;i++) debug<<dis[i]<<" "; debug<<endl;*/
    for(int i=1;i<=n;i++) debug<<top[i]<<" "; debug<<endl;
    for(int i=1;i<=n;i++) debug<<id[i]<<" "; debug<<endl;
