...
std::vector<bool> f[maxl][maxl][maxn];
bool dp[maxl][maxl];
int n,li;
...
inline void getans() {
    for(int len=1;len<=li;len++)
        for(int st=1;st+len-1<=li;st++) {
            ...
            for(int p=1;p<=n;p++) { // p is our pairing string .
                for(int k=1;k<=lt[p]&&k<=len;k++) {
                    ...
                    for(int l=st;l<ed;l++) {
                        if( l == ed - 1 ) f[st][ed][p][k] = f[st][ed][p][k] | f[st][l][p][k-1];
                        else f[st][ed][p][k] = f[st][ed][p][k] | ( f[st][l][p][k-1] && dp[l+1][ed-1] );
                    }
                }
            }
