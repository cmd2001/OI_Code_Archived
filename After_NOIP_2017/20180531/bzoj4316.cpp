#include<cstdio>
#include<algorithm>
const int maxn=5e4+1e2,maxe=6e4+1e2;
...
inline void solve(int rt,int pos) {
    static int rng[maxn],cnt,p[2];
    cnt = 0 , p[0] = p[1] = 0;
    for(int i=pos;i!=fa[rt];i=fa[i]) rng[++cnt] = i;
    g[1][0] = f[pos][0] , g[1][1] = -inf;
    for(int i=2,p=rng[i];i<=cnt;p=rng[++i]) {
        g[i][0] = f[p][0] + std::max(g[i-1][0],g[i-1][1]) ,
        g[i][1] = f[p][1] + g[i-1][0];
    }
    p[0] = std::max(f[rt][0],g[cnt][0]) , p[1] = std::max(f[rt][1],g[cnt][1]);
...
inline void dfs(int pos) {
    low[pos] = dfn[pos] = ++dd , f[pos][0] = 0 , f[pos][1] = 1;
