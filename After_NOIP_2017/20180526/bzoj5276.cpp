...
const int maxn=5e3+1e2,lim=5e3;
const int mod=1<<30;
...
inline void init() {
    ...
    memset(fa+1,0,n<<3) , memset(fb+1,0,n<<3) , memset(fc+1,0,n<<3) , memset(deg+1,0,n<<2) , ecnt = 0;
...
int main() {
    sieve() , a = getint() , b = getint() , c = getint() , init() , getf(fa,a) , getf(fb,b) , getf(fc,c);
