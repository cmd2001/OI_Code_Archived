...
inline int chain(int a,int b) {
    ...
    //debug<<"before last ret = "<<ret<<endl;
...
inline void check() {
    for(int i=1;i<=n;i++)
        if( query(cov[top[i]],num[i],num[i]) )
            debug<<"sum error"<<endl,exit(0);
}
...
inline void solve(int ql,int qr,int al,int ar) {
    ...
    //debug<<"amid = "<<amid<<endl;
    int cntl = 0 , cntr = 0;
    for(int i=ql;i<=qr;i++) {
        if( ns[i].tpe ) {
            int kk = chain(ns[i].a,ns[i].b);
            //debug<<"a = "<<ns[i].a<<"b = "<<ns[i].b<<endl;
            //debug<<"kk = "<<kk<<endl;
            if( ns[i].k > kk ) {
                //debug<<"to left"<<endl;
            ...
            if( ns[i].val > amid ) {
                //debug<<"pos = "<<ns[i].pos<<"val = "<<srt[ns[i].val]<<endl;
                //debug<<"add = "<<ns[i].add<<endl;
...
int main() {
    ...
    //for(int i=1;i<=qlen;i++) debug<<ns[i].val<<" ";debug<<endl;
