...
struct Splay {
    ...
    int sum[maxn],rev[maxn],swp[maxn],fil[maxn],lmin[maxn],rmin[maxn],lmax[maxn],rmax[maxn];
    
    inline void applyfil(int pos,int col) {
        if( !pos ) return;
        rev[pos] = swp[pos] = 0 , dat[pos] = fil[pos] = col;
        lmin[pos] = lmax[pos] = rmin[pos] = rmax[pos] = sum[pos] = siz[pos] * col;
    }
    ...
    inline void push(int pos) {
        if( fil[pos] ) applyfil(ch[pos][0],fil[pos]) , applyfil(ch[pos][1],fil[pos]) , fil[pos] = 0;
    ...
    inline void modifyfil(int l,int r,int col) {
        int s = getseg(l,r);
        applyfil(s,col) , upchain(s);
    }
...
int main() {
    ...
    for(int i=1,l,r;i<=m;i++) {
        scanf("%s%d%d",o,&l,&r);
        if( *o == 'R' ) scanf("%s",o) , tree.modifyfil(l,r,*o==')'?1:-1);
        else if( *o == 'S' ) tree.modifyrev(l,r);
        else if( *o == 'I' ) tree.modifyswp(l,r);
