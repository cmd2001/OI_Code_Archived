...
int s[maxn],t[maxe],nxt[maxe],f[maxe],sou[maxn];
lli c[maxe],dis[maxn];
bool inq[maxn];
...
int main() {
    ...
    for(int i=1;i<=k;i++) singledge(st,i,1,ine[i]);
    for(int i=1;i+k<=n;i++) singledge(i,i+k,1,ine[i+k]);
    
    /*singledge(st,1,k-me,0) , singledge(n,ed,lim,0);
    for(int i=1;i<n;i++) singledge(i,i+1,i<k?k-me:lim,0);
    for(int i=1;i+k<=n;i++) singledge(i,i+k,1,ine[i]);
    for(int i=n-k+1;i<=n;i++) singledge(i,ed,1,ine[i]);*/
