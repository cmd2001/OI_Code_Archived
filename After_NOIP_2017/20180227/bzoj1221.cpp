...
int main() {
    ...
    scanf("%d%d%d%d%d%d",&t,&m,&n,&p,&f,&s) , ++n , ++m;
    st = t * 2 + 1 , ed = t * 2 + 2;
    for(int i=1;i<=t;i++)
        scanf("%d",in+i);
    for(int i=1;i<=t;i++) {
        ...
        if( i + m <= t ) addedge(cov(i,1),cov(i+m,0),f,int_inf);
        if( i + n <= t ) addedge(cov(i,1),cov(i+n,0),s,int_inf);
        if( i + 1 <= t ) addedge(cov(i,1),cov(i+1,1),0,int_inf);
    }
