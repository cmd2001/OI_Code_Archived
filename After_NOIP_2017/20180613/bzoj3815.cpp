...
struct KDTree {
    ...
    inline void maintain(int pos) {
        ...
        if( lson[pos] ) for(int i=0;i<3;i++) mi[pos][i] = std::min( mi[pos][i] , mi[lson[pos]][i] ) , mx[pos][i] = std::max( mx[pos][i] , mx[lson[pos]][i] );
        if( rson[pos] ) for(int i=0;i<3;i++) mi[pos][i] = std::min( mi[pos][i] , mi[rson[pos]][i] ) , mx[pos][i] = std::max( mx[pos][i] , mx[rson[pos]][i] );
    }
    inline void maintain_chain(int pos) {
        while(pos) maintain(pos) , pos = fa[pos];
    }
    inline int build(int pos,int dir,int ll,int rr) {
        const int mid = ( ll + rr ) >> 1; cmp = dir , std::nth_element(ps+ll,ps+mid,ps+rr+1);
    ...
    inline double far(int pos,const Circle &c) {
        double su = 0;
        for(int i=0;i<3;i++) su += sq( std::max( std::fabs(c.d[i]-mi[pos][i]) , std::fabs(c.d[i]-mx[pos][i] ) ) );
        return std::sqrt(su);
    }
...
int main() {
    n = getint() , m = getint() , scanf("%lf%lf",&a,&b) , init();
    for(int i=1;i<=m;i++) {
        if( !getint() ) {
